...
### 1.2.1
* Fix bulk-job token case mismatch.
* Persist active bulk jobs in user metadata to avoid object-cache eviction.

### 1.2.0
* Apply selected suggestions directly to Yoast metadata.
* Collapse duplicate metadata inputs by default and offer a refresh action.
...
* Keep provider requests below common web-server timeout limits.
